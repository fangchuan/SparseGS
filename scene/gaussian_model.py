...
class GaussianModel:
    ...
    def compute_3D_filter(self, cameras):
        ...
        for camera in cameras:
            ...
            valid_depth = xyz_cam[:, 2] > 0.01 # TODO remove hard coded value
    ...
    def  ft_cameras_setup(self, training_args, num_cams: int = 32):
        from scene.camera_optimizer import CameraOptModule
        batch_size = 1
        if training_args.pose_opt:
            self.pose_adjust = CameraOptModule(num_cams).to("cuda")
            self.pose_adjust.zero_init()
            self.pose_optimizer = torch.optim.Adam(
                    self.pose_adjust.parameters(),
                    lr=training_args.pose_opt_lr * math.sqrt(batch_size),
                    weight_decay=training_args.pose_opt_reg,
                )
        
            
            self.pose_scheduler_args = get_expon_lr_func(lr_init=training_args.pose_opt_lr*self.spatial_lr_scale,
                                                    lr_final=training_args.pose_opt_lr_final*self.spatial_lr_scale,
                                                    lr_delay_mult=training_args.position_lr_delay_mult,
                                                    max_steps=training_args.position_lr_max_steps)

        # if training_args.pose_noise > 0.0:
        #     self.pose_perturb = CameraOptModule(num_cams).to("cuda")
        #     self.pose_perturb.random_init(training_args.pose_noise)
